 ...
 */
#ifndef FLASHINFER_GROUP_GEMM_NVFP4_GROUPWISE_SM100_CUH_
#define FLASHINFER_GROUP_GEMM_NVFP4_GROUPWISE_SM100_CUH_
...
template <int ScaleGranularity, typename ScaleConfig, typename ElementA, typename ElementB,
          ...
          typename LayoutSFB>
__global__ void compute_sm100_cutlass_nvfp4_group_gemm_args(
...
cudaError_t CutlassNVFP4GroupwiseScaledGroupGEMMSM100(
...
// DTypeInA: data type of input matrix A (m x k)
// DTypeInB: data type of input matrix B (k x n) -- block-scaled NVFP4 format
// DTypeSFA: data type of scale factors for A (m x (k / ScaleGranularity))
// DTypeSFB: data type of scale factors for B (n x (k / ScaleGranularity))
// DTypeOut: data type of output matrix D (m x n)
// DTypeInAName, DTypeInBName, DTypeSFAName, DTypeSFBName, DTypeOutName:
//     stringified type names used to form unique C++ symbol names

#define INSTANTIATE_GROUP_GEMM_NVFP4_GROUPWISE_SM100(                                                                                                                  \
  ...
  inline cudaError_t                                                                                                                                                   \
      CutlassNVFP4GroupwiseScaledGroupGEMMSM100_##TileM##_##TileN##_##TileK##_##DTypeInAName##_##DTypeInBName##_##DTypeSFAName##_##DTypeSFBName##_##DTypeOutName(      \
          void* int_buffer, size_t int_buffer_size_in_bytes, void* float_buffer,                                                                                       \
    ...
    using CollectiveEpilogue = typename cutlass::epilogue::collective::CollectiveBuilder<                                                                              \
        cutlass::arch::Sm100, cutlass::arch::OpClassTensorOp, ThreadBlockShape,                                                                                        \
    ...
    config.numAttrs = 1;                                                                                                                                               \
    config.attrs = attrs;                                                                                                                                              \
    auto prepare_args_kernel = compute_sm100_cutlass_nvfp4_group_gemm_args<                                                                                            \
    ...
    auto workspace_ptr = float_allocator.aligned_alloc<void>(                                                                                                          \
        workspace_size, 16, "sm100_nvfp4_groupwise_group_gemm_float_workspace");                                                                                       \
    ...
    return cudaSuccess;                                                                                                                                                \
                                                                                                                                                                       ...
                                                                                                                                                                       \
  template <>                                                                                                                                                          \
  cudaError_t CutlassNVFP4GroupwiseScaledGroupGEMMSM100<TileM, TileN, TileK, DTypeInA, DTypeInB,                                                                       \
                                                        DTypeSFA, DTypeSFB, DTypeOut>(                                                                                 \
      void* int_buffer, size_t int_buffer_size_in_bytes, void* float_buffer,                                                                                           \
      size_t float_buffer_size_in_bytes, DTypeInA* A, DTypeInB* B, DTypeSFA* SFA, DTypeSFB* SFB,                                                                       \
      DTypeOut* D, float* alpha, int* m_indptr, int n, int k, int num_groups, cudaStream_t stream,                                                                     \
      int device_id) {                                                                                                                                                 \
    return CutlassNVFP4GroupwiseScaledGroupGEMMSM100_##TileM##_##TileN##_##TileK##_##DTypeInAName##_##DTypeInBName##_##DTypeSFAName##_##DTypeSFBName##_##DTypeOutName( \
...
#endif  // FLASHINFER_GROUP_GEMM_NVFP4_GROUPWISE_SM100_CUH_
